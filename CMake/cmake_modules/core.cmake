...
macro(add_sb_python_package)
  ...
  set(_tmp ${ARGN})
  if ("${_python_pkg}" STREQUAL "")
    string(TOLOWER ${_name} _python_pkg)
    list(APPEND _tmp PYTHON_PACKAGE ${_python_pkg})
  endif()
  add_sb_package(${_tmp})
endmacro()
...
macro(add_sb_package)
  ...
  if(SB_ENABLE_${uc_package_name} STREQUAL "SYSTEM")
    # If we aren't dealing with a Python package use find_package(...)
    if ("${_python_pkg}" STREQUAL "")
        if(DEFINED _version)
      ...
      if(_installed_version)
        if(NOT "" STREQUAL "${_version}")
          if(_installed_version VERSION_EQUAL _version OR
            ...
            message("[INFO] We have ${lc_package_name} ${_installed_version} installed")
          else()
            message(FATAL_ERROR "[sb:error] Unable to find required version Python package ${_python_pkg}, ${_version} is required, ${_installed_version} is installed")
          endif()
        else()
          message(WARNING "[sb:warning] No required version specified for Python package ${_python_pkg}")
        endif()
      else()
          message(FATAL_ERROR "[sb:error] Unable to find Python package ${_python_pkg}")
      endif()
    endif()
  endif()
endmacro()
