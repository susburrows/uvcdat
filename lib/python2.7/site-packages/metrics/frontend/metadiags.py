...
def generatePlots(modelpath, obspath, outpath, pname, sets=None):
   ...
   #                  print obsfname
         ...
         if setnum != 'topten' and setnum != 'so':
            ...
            v5 = [x for x in vl if x in ['SWCF', 'LWCF', 'PRECT', 'AODVIS', 'FSNS', 'FLDS_SOUTH', 'LHFLX']]
            v6 = [x for x in vl if x in ['SURF_STRESS']]
            v7 = [x for x in vl if x in ['SHFLX_SOUTH', 'QFLX_SOUTH', 'SURF_WIND_SOUTH', 'CLDTOT_SOUTH']]
            print 'vl in:' ,vl
            if v4 != []:
               realsetnum = 4
               vl1 = list(set(v4) & set(vl))
