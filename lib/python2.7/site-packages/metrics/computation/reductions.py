...
def reduce2scalar_seasonal_zonal_region( mv, seasons=seasonsyr, region=None, vid=None, gw=None ):
    mvr = select_region(mv, region)
    mvrssz = reduce2scalar_seasonal_zonal( mv, seasons=seasonsyr, latmin=region[0], latmax=region[1], vid=vid, gw=gw)
    return mvrssz
...
def reduce2scalar_seasonal_zonal( mv, seasons=seasonsyr, latmin=-90, latmax=90, vid=None, gw=None ):
    ...
    mvseas = calculate_seasonal(mv2, seasons)
    if mvseas is None:
...
def reduce2level_region( mv, seasons=None, region=None, vid=None ):
    mvr = select_region(mv, region)
    mvrl = reduce2level(mv, seasons=None, vid=None)
    return mvrl
...
def reduce2levlat_seasonal_region( mv, seasons=seasonsyr, region=None, vid=None ):
    mvr = select_region(mv, region)
    mvllsr = reduce2levlat_seasonal( mvr, seasons=seasonsyr, vid=vid )
    return mvllsr
...
def reduce2lat_seasonal_region( mv, seasons=seasonsyr, region=None, vid=None ):
    mvr = select_region(mv, region)
    mvrls = reduce2lat_seasonal( mv, seasons, vid)
    return mvrls
...
# TODO check this for redundancies with my introduced functions
def reduce_time_space_seasonal_regional( mv, season=seasonsyr, region=None, vid=None ):
    ...
    mvtsav.id = calculate_seasonal(mvsav, season)
...
def reduce_seasonal_region( mv, season=seasonsyr, region=None, vid=None ):
    """Calculates the seasonal mean, and selects data within the specified region.
    No change on level axis.
    The season should be a cdutil.times.Seasons object.
    The region may be a string defining a region in defines.py, or it may be a list of four numbers
    as in defines.py.  That is, it would take the form [latmin,latmax,lonmin,lonmax].
    """
    ...
    mvt = calculate_seasonal_climatology(mv, season)

    # Select lat-lon region
    mvtreg = select_region(mvt, region)

    # TODO: refactor this code that checks units into another function
    mvtreg.id = vid
    if hasattr(mv,'units'):
        mvtreg.units = mv.units

    # >>> special ad-hoc code.  The target units should be provided in an argument, not by this if statement>>>>
        if mvtreg.units=="Pa" or mvtreg.units.lower()=="pascal" or mvtreg.units.lower()=="pascals":
            mvtreg = convert_variable( mvtreg, "millibar" )

    return mvtreg

# TODO: call reduce_seasonal_region from reduce_seasonal_region_level to reduce cut-and-paste code
def reduce_seasonal_region_level( mv, season, region, level, vid=None ):
    """Calculates the seasonal mean at a specified model level, and
    selects data within the specified region.
    The season should be a cdutil.times.Seasons object.
    The region may be a string defining a region in defines.py, or it may be a list of four numbers
    as in defines.py.  That is, it would take the form [latmin,latmax,lonmin,lonmax].
    """
    ...
    # Select on level axis
    select_lev( mv, level )

    # Select lat-lon region
    mvreg = select_region(mvsub, region)

    # Calculate seasonal climatology
    mvtreg = calculate_seasonal_climatology(mvreg, season)

    # TODO: do all the operations above retain metadata?  If so, some of the following lines may be redundant.
    mvtreg.id = vid
    if hasattr(mv,'units'):
        mvtreg.units = mv.units
    # TODO >>> special ad-hoc code.  The target units should be provided in an argument, not by this if statement>>>>
        if mvtreg.units=="Pa" or mvtreg.units.lower()=="pascal" or mvtreg.units.lower()=="pascals":
            mvtreg = convert_variable( mvtreg, "millibar" )
...
def reduce2latlon_seasonal( mv, season=seasonsyr, vid=None, exclude_axes=[], region=None ):
...
# TODO: refactor/improve this recurring code block
    if len(axes_string)>2:
...
def reduce_time_seasonal_region( mv, seasons=seasonsyr, region=None, vid=None ):
    mvseas = reduce_time_seasonal(mv, seasons, vid)
...
def reduce_time_seasonal( mv, season=seasonsyr, vid=None ):
...
def select_lev( mv, slev ):
    ...
    if levax == mv.getAxisList()[0]:
        ...
        return None
    mvs = delete_singleton_axis(mvs, vid=levax)
    return mvs
