...
from amwg import amwg_plot_spec
...
def reduced_variables_press_lev( filetable, varid, season, region=None, filefilter=None, rf=None):
    ...
    if rf is None:  # default can't be specified in the args because it depends on season
        rf = (lambda x,vid=None,season=season: reduce2levlat_seasonal_region(x,season, region, vid=vid))
    reduced_varlis = [
        reduced_variable(
            variableid=varid, filetable=filetable, season=season, region=region, filefilter=filefilter,
            reduction_function=rf ) ]
    reduced_variables = { v.id():v for v in reduced_varlis }
    return reduced_variables
def reduced_variables_hybrid_lev( filetable, varid, season, region=None, filefilter=None, rf=None, rf_PS=None ):
    ...
    if filetable is None:
        return {}
    if rf is None:  # default can't be specified in the args because it depends on season
        rf=(lambda x,vid=None,season=season, region=region: reduce2levlat_seasonal_region(x,season, region, vid=vid))
    if rf_PS is None:  # default can't be specified in the args because it depends on season
        rf_PS=(lambda x,vid=None,season=season, region=region: reduce2lat_seasonal_region(x,season, region, vid=vid))
    reduced_varlis = [
        reduced_variable(
            variableid=varid, filetable=filetable, season=season, region=region, filefilter=filefilter,
            reduction_function=rf ),
        reduced_variable(      # hyam=hyam(lev)
            variableid='hyam', filetable=filetable, season=season, region=region, filefilter=filefilter,
            reduction_function=(lambda x,vid=None, region=region: select_region(x, region)) ),
        reduced_variable(      # hybm=hybm(lev)
            variableid='hybm', filetable=filetable, season=season, region=region, filefilter=filefilter,
            reduction_function=(lambda x,vid=None, region=region: select_region(x, region)) ),
        reduced_variable(
            variableid='PS', filetable=filetable, season=season, region=region, filefilter=filefilter,
...
class amwg_plot_set1(amwg_plot_spec):
    ...
    # TODO: this should be read from amwgmaster.py (?) -- SMB
    table_row_specs = [
        { 'var':'RESTOM'},
        { 'var':'RESSURF'},
    ...
    # SMB -- I commented these lines out because they are not needed when the
    # standard "regions" definition is used.
    ...
    class myrow:
        ...
        def __init__( self, filetable1, filetable2, seasonid='ANN', region='global', var='TREFHT',
            ...
            # Commented out these lines because 'global' is already the default for the region argument
        ...
        def diff(self, m1, m2 ):
            ...
            if m1==-999.000 or m2==-999.000:
                ...
                return m1-m2
        def mean_lev( self, filetable, ffilt, region, gw ):
            ...
            if hybrid: # hybrid level coordinates
                ...
                derived_variables = { vid1: derived_var(
                        ...
                        vid=vidm1, inputs=[vidl1], func=\
                            ...
                            (lambda x,vid=None,season=self.season,region=region,gw=gw:
                                 reduce2scalar_seasonal_zonal_region(x,season,region,vid=vid,gw=gw) ) )
                                      }
                ...
                reduced_variables = reduced_variables_press_lev(
                    filetable, self.var, self.seasonid, filefilter=ffilt, rf=\
                        ...
                        #          reduce2scalar_seasonal_zonal_level(x,season,dom0,dom1,level=ulev,vid=vid,gw=gw) )
                         (lambda x,vid=None,season=self.season,region=region,gw=gw:
                                  reduce2scalar_seasonal_zonal_level(x,season,region,level=ulev,vid=vid,gw=gw) )                    )
                derived_variables = {}
