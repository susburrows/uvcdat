    ...
    def stdvar2var( cls, varnom, filetable, season, reduction_function ):
        ...
        for ivn in invarnoms:
            rv = reduced_variable( variableid=ivn, filetable=filetable, season=season,
                                   region=region, reduction_function=reduction_function )
            rvs.append(rv)
...
class amwg_plot_set3(amwg_plot_spec,basic_id):
    ...
    def __init__( self, filetable1, filetable2, varnom, seasonid=None, region=None, aux=None ):
        ...
        plot_spec.__init__(self,seasonid)
        self.season = cdutil.times.Seasons(self._seasonid)  # note that self._seasonid can differ froms seasonid
        self.region = self.interpret_region(region)
        if not self.computation_planned:
...
#        print "amwg plot set 4 listvars=",listvars
    ...
    def reduced_variables_press_lev( self, filetable, varid, seasonid, ftno=None ):
        return reduced_variables_press_lev( filetable, varid, self.season, self.region )
    def reduced_variables_hybrid_lev( self, filetable, varid, seasonid, ftno=None ):
        return reduced_variables_hybrid_lev( filetable, varid, self.season, self.region )
    def plan_computation( self, filetable1, filetable2, varid, seasonid ):
        ...
        if hybrid1:
            reduced_variables_1 = self.reduced_variables_hybrid_lev( filetable1, varid, seasonid, self.region )
        else:
            reduced_variables_1 = self.reduced_variables_press_lev( filetable1, varid, seasonid, self.region )
        if hybrid2:
            reduced_variables_2 = self.reduced_variables_hybrid_lev( filetable2, varid, seasonid, self.region )
        else:
            reduced_variables_2 = self.reduced_variables_press_lev( filetable2, varid, seasonid, self.region )
...
class amwg_plot_set6(amwg_plot_spec):
    ...
    def STRESS_rvs(self, filetable, rvars, seasonid, vardict, region):
        ...
        for var in rvars:
            if var in ['TAUX','TAUY'] and filetable.filefmt.find('CAM')>=0:
                ...
                if filetable.has_variables(['OCNFRAC']):
                    ...
                    reduced_vars.append( reduced_variable(
                            ...
                            reduction_function=(lambda x,vid=None:
                                                minusb(reduce_seasonal_region( mv, self.season, region, vid)) ) ))
                    needed_derivedvars.append(var)
                    reduced_vars.append( reduced_variable(
                            variableid='OCNFRAC', filetable=filetable, season=self.season,
                            reduction_function=(lambda x,vid=None:
                                                    reduce_seasonal_region( mv, self.season, region, vid) ) ))
                ...
                reduced_vars.append( reduced_variable(
                        variableid=var, filetable=filetable, season=self.season,
                        reduction_function=(lambda x,vid=None:
                                                reduce_seasonal_region( x, self.season, region, vid) ) ))
    ...
    def plan_computation_normal_contours( self, filetable1, filetable2, varid, seasonid, aux=None ):
        ...
        new_reducedvars, needed_derivedvars = self.STRESS_rvs(filetable1, rvars1, seasonid, vardict1, self.region)
        reduced_varlis += new_reducedvars
        self.reduced_variables = { v.id():v for v in reduced_varlis }
        self.STRESS_dvs( filetable1, needed_derivedvars, seasonid, vardict1, vid_cont1, vars_vec1 )
        new_reducedvars, needed_derivedvars = self.STRESS_rvs(filetable2, rvars2, seasonid, vardict2, self.region)
        reduced_varlis += new_reducedvars
        ...
        for FT in self.filetables:
            ...
            for i in range(1, 13):
                ...
                RV = reduced_variable(variableid = varid, 
                                      filetable = FT, 
                                      season = cdutil.times.Seasons(VID[2]), 
                                      reduction_function =  RF) # TODO: add region where this syntax is used
