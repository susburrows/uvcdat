...
class amwg_plot_set2(amwg_plot_spec):
    ...
    def plan_computation( self, filetable1, filetable2, varid, seasonid, region=None ):
        ...
        self.reduced_variables = {
            'FSNS_1': reduced_variable(
                ...
                reduction_function=(lambda x,vid: select_region( x, region)) ),
            'FSNS_ANN_latlon_1': reduced_variable(
                variableid='FSNS',
                filetable=filetable1, season=self.season,
                reduction_function=(lambda x, vid: reduce2latlon_region(x, self.season, region, vid)) ),
...
class amwg_plot_set3(amwg_plot_spec,basic_id):
    ...
    def plan_computation( self, filetable1, filetable2, varnom, seasonid, region ):
        zvar = reduced_variable(
            variableid=varnom,
            filetable=filetable1, season=self.season,
            reduction_function=(lambda x,vid=None: reduce2lat_seasonal_region(x,self.season,region,vid=vid)) )
    ...
    def plan_computation_normal_contours( self, filetable1, filetable2, varnom, seasonid, region=None, aux=None ):
        ...
        if varnom in filetable1.list_variables():
            vid1,vid1var = self.vars_normal_contours(
                filetable1, varnom, seasonid, region, aux=None )
        ...
        if filetable2 is not None and varnom in filetable2.list_variables():
            vid2,vid2var = self.vars_normal_contours(
                filetable2, varnom, seasonid, region, aux=None )
        elif varnom in self.standard_variables.keys():
    ...
    def vars_normal_contours( self, filetable, varnom, seasonid, region=None, aux=None ):
        reduced_varlis = [
            reduced_variable(
                ...
                reduction_function=(lambda x,vid: reduce_seasonal_region( x, self.season, region, vid) ) ),
            reduced_variable(
    ...
    def plan_computation_level_surface( self, filetable1, filetable2, varid, seasonid, region, aux ):
        ...
        reduced_varlis = [
            reduced_variable(  # var=var(time,lev,lat,lon)
                ...
                reduction_function=(lambda x,vid: reduce_time_seasonal_region( x, self.season, region, vid ) ) ),
            reduced_variable(   # hyam=hyam(lev)
                variableid='hyam', filetable=filetable1, season=self.season,
                reduction_function=(lambda x,vid=None: select_region( x, region)) ),
            ...
            reduced_variable(     # ps=ps(time,lat,lon)
                ...
                reduction_function=(lambda x,vid=None: reduce_time_seasonal_region( x, self.season, region, vid ) ) ) ]
        # vid1 = varid+'_p_1'
        # vidl1 = varid+'_lp_1'
...
class amwg_plot_set6(amwg_plot_spec):
    ...
    def STRESS_rvs(self, filetable, rvars, seasonid, vardict, region='Global'):
    ...
    def plan_computation_normal_contours( self, filetable1, filetable2, varid, seasonid, region=None, aux=None ):
        ...
        new_reducedvars, needed_derivedvars = self.STRESS_rvs(filetable1, rvars1, seasonid, vardict1, region)
        reduced_varlis += new_reducedvars
        self.reduced_variables = { v.id():v for v in reduced_varlis }
        self.STRESS_dvs( filetable1, needed_derivedvars, seasonid, vardict1, vid_cont1, vars_vec1 )
        new_reducedvars, needed_derivedvars = self.STRESS_rvs(filetable2, rvars2, seasonid, vardict2, region)
        reduced_varlis += new_reducedvars
        ...
        for FT in self.filetables:
            ...
            for i in range(1, 13):
                ...
                RV = reduced_variable(variableid = varid, 
                                      ...
                                      reduction_function =  (lambda x, vid=id2str(VID), month=VID[2]:reduce2lat_seasonal_region(x, seasons=cdutil.times.Seasons(month), region, vid=vid)))
...
class amwg_plot_set9(amwg_plot_spec): 
    ...
    def plan_computation( self, filetable1, filetable2, varid, seasonid ):
        ...
        rv_1 = reduced_variable(variableid=varid, filetable=filetable1, season=vid1_season,
                                reduction_function=( lambda x, vid=vid1:reduce_seasonal_region(x, vid1_season, region, vid=vid)) )
        
        rv_2 = reduced_variable(variableid=varid, filetable=filetable2, season=vid2_season,
                                reduction_function=( lambda x, vid=vid2:reduce_seasonal_region(x, vid2_season, region, vid=vid)) )
        ...
        for FT in [filetable1, filetable2]:
            ...
            for i in range(1, 13):
                ...
                RV = reduced_variable(variableid = varid, 
                                      ...
                                      reduction_function =  (lambda x, vid=id2str(VID), month = VID[2]:reduce2scalar_seasonal_zonal(x, seasons=cdutil.times.Seasons(month), vid=vid)))
            ...
            for season in self.seasons:
                for var in self.vars:
                    ...
                    RV = reduced_variable( variableid=var, 
                                           ...
                                           reduction_function=( lambda x, vid=VID: select_region( x, region)) )
                    self.reduced_variables[VID] = RV      
                    VIDs += [VID]              
        ...
        for dt, ft in zip(self.datatype, self.filetables):
            ...
            for month in self.months:
                ...
                RV = reduced_variable( variableid=varid,
                                       ...
                                       reduction_function=(lambda x, vid=VID, month=VID[2]: reduce2level_region(x, seasons=month, region, vid=vid) ) )
                VID = id2str(VID)
                self.reduced_variables[VID] = RV      
                VIDs[dt] += [VID]              
