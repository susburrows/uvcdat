...
# SMB+
# Added lines for debugging interactively in ipython
# see: http://stackoverflow.com/questions/16867347/step-by-step-debugging-with-ipython
# First import the embed function
from IPython.terminal.embed import InteractiveShellEmbed
import inspect

# Wrap it in a function that gives me more context.
# Call this function whereever you want to drop into the shell.
def ipsh():
    banner_msg='Dropping into IPython'
    exit_msg = 'Leaving interpreter, back to program.'

    ipshell = InteractiveShellEmbed(config=None, banner1=banner_msg, exit_msg=exit_msg)
    ...
    def plan_computation( self, filetable1, filetable2, varid, seasonid ):
        ...
        self.reduced_variables = {
            ...
            'FSNS_ANN_latlon_1': reduced_variable(
                ...
                reduction_function=(lambda x, vid: reduce2latlon(x, self.season, vid)) ),
            ...
            'FLNT_1': reduced_variable(
                variableid='FLNT',filetable=filetable1,
                reduction_function=(lambda x,vid: x) ),
            ...
            'QFLX_1': reduced_variable(
                variableid='QFLX',filetable=filetable1,
                reduction_function=(lambda x,vid: x) ),
...
class amwg_plot_set3(amwg_plot_spec,basic_id):
    ...
    def plan_computation( self, filetable1, filetable2, varnom, seasonid ):
        zvar = reduced_variable(
            ...
            reduction_function=(lambda x,vid=None: reduce2lat_seasonal_region(x,self.season,self.region,vid=vid)) )
...
#        print "amwg plot set 4 listvars=",listvars
    ...
    def reduced_variables_press_lev( self, filetable, varid, seasonid, ftno=None ):
        return reduced_variables_press_lev( filetable, varid, self.season, region=self.region )
    def reduced_variables_hybrid_lev( self, filetable, varid, seasonid, ftno=None ):
        return reduced_variables_hybrid_lev( filetable, varid, self.season, region=self.region )
    def plan_computation( self, filetable1, filetable2, varid, seasonid ):
...
class amwg_plot_set5and6(amwg_plot_spec):
    ...
    def vars_normal_contours( self, filetable, varnom, seasonid, aux=None ):
        ...
        vid = rv.dict_id( varnom, seasonid, filetable, region=self.region )
        vidvar = rv.dict_id( varnom+'_var', seasonid, filetable, region=self.region ) # variance
        return vid, vidvar
        ...
        varid,rvs,dvs = self.stdvar2var(
            varnom, filetable, self.season,\
                (lambda x,vid,season=self.season, region=self.region:
                     reduce_seasonal_region(x, season, vid, region, exclude_axes=['isccp_prs','isccp_tau']) ))
    ...
    def plan_computation_level_surface( self, filetable1, filetable2, varid, seasonid, aux ):
        ...
        reduced_varlis = [
            reduced_variable(  # var=var(time,lev,lat,lon)
                ...
                reduction_function=(lambda x,vid: reduce_time_seasonal_region( x, self.season, self.region, vid ) ) ),
            ...
            reduced_variable(     # ps=ps(time,lat,lon)
                ...
                reduction_function=(lambda x,vid=None: reduce_time_seasonal_region( x, self.season, self.region, vid ) ) ) ]
        # vid1 = varid+'_p_1'
        # vidl1 = varid+'_lp_1'
...
class amwg_plot_set6(amwg_plot_spec):
            ...
            if var in ['TAUX','TAUY'] and filetable.filefmt.find('CAM')>=0:
                ...
                if filetable.has_variables(['OCNFRAC']):
                    ...
                    reduced_vars.append( reduced_variable(
                            variableid=var, filetable=filetable, season=self.season, region=self.region,
                            #reduction_function=(lambda x,vid=var+'_nomask':
                            reduction_function=(lambda x,vid=None:
                                                minusb(reduce_seasonal_region( x, self.season, self.region, vid)) ) ))
                    needed_derivedvars.append(var)
                    reduced_vars.append( reduced_variable(
                            variableid='OCNFRAC', filetable=filetable, season=self.season, region=self.region,
                            reduction_function=(lambda x,vid=None:
                                                    reduce_seasonal_region( x, self.season, self.region, vid) ) ))
                    ...
                    needed_derivedvars.append(var)
                    reduced_vars.append( reduced_variable(
                            variableid='ORO', filetable=filetable, season=self.season, region=self.region,
                            reduction_function=(lambda x,vid=None:
                                                    reduce_seasonal_region( x, self.season, self.region, vid) ) ))
                else:
    ...
    def STRESS_dvs( self, filetable, dvars, seasonid, vardict, vid_cont, vars_vec ):
            ...
            if var in ['TAUX','TAUY']:
                ...
                if filetable.has_variables(['OCNFRAC']):
                    ...
                    new_derived_var = derived_var( vid=vid, inputs=[tau,ocn_frac], outputs=[var],
                                                    region=self.region, func=mask_OCNFRAC )
                    ...
                    new_derived_var = derived_var( vid=vid, inputs=[tau,oro], outputs=[var],
                                                    region=self.region,func=mask_ORO )
                    derived_vars.append( new_derived_var )
                    vardict[var] = vid
                    self.derived_variables[vid] = new_derived_var
                else:
        ...
        if tuple(vid_cont) and vid_cont[0]=='dv':  # need to compute STRESS_MAG from TAUX,TAUY
            ...
            new_derived_var = derived_var( vid=vid_cont, inputs=[tau_x,tau_y], func=abnorm, region=self.region )
            derived_vars.append( new_derived_var )
...
class amwg_plot_set7(amwg_plot_spec):
    ...
    number = '7'
    def __init__( self, filetable1, filetable2, varid, seasonid=None, regionid=None, aux=slice(0,None) ):
        """filetable1, filetable2 should be filetables for model and obs.
        varid is a string identifying the variable to be plotted, e.g. 'TREFHT'.
        seasonid is a string such as 'DJF'."""
    ...
    def _all_variables( filetable1, filetable2=None ):
        ...
        return allvars
    def plan_computation( self, filetable1, filetable2, varid, seasonid, aux=slice(0,None) ):
        """Set up for a lat-lon polar contour plot.  Data is averaged over all other axes."""

        reduced_varlis = [
            reduced_variable(
                variableid=varid, filetable=filetable1, season=self.season, region=self.region,
                reduction_function=(lambda x,vid: reduce2latlon_seasonal( x(latitude=aux, longitude=(0, 360)), self.season, vid, region=self.region ) ) ),
            reduced_variable(
                variableid=varid, filetable=filetable2, season=self.season, region=self.region,
                reduction_function=(lambda x,vid: reduce2latlon_seasonal( x(latitude=aux, longitude=(0, 360)), self.season, vid, region=self.region ) ) )
            ]
        self.reduced_variables = { v.id():v for v in reduced_varlis }
        vid1 = rv.dict_id( varid, seasonid, filetable1 )
        ...
        for FT in self.filetables:
            ...
            for i in range(1, 13):
                ...
                RV = reduced_variable(variableid = varid, 
                                      ...
                                      region = self.region,
                                      reduction_function =  (lambda x, vid=id2str(VID), month=VID[2]:reduce2lat_seasonal_region(x, seasons=cdutil.times.Seasons(month), region=self.region, vid=vid)))
...
class amwg_plot_set9(amwg_plot_spec): 
    ...
    def plan_computation( self, filetable1, filetable2, varid, seasonid ):
        ...
        rv_1 = reduced_variable(variableid=varid, filetable=filetable1, season=vid1_season, region=self.region,
                                reduction_function=( lambda x, vid=vid1:reduce_seasonal_region(x, vid1_season, self.region, vid=vid)) )
        
        rv_2 = reduced_variable(variableid=varid, filetable=filetable2, season=vid2_season, region=self.region,
                                reduction_function=( lambda x, vid=vid2:reduce_seasonal_region(x, vid2_season, self.region, vid=vid)) )
        ...
        for FT in [filetable1, filetable2]:
            ...
            for i in range(1, 13):
                ...
                RV = reduced_variable(variableid = varid, 
                                      ...
                                      region = self.region,
                                      reduction_function =  (lambda x, vid=id2str(VID), month = VID[2]:reduce2scalar_seasonal_zonal_region(x, seasons=cdutil.times.Seasons(month), region=self.region, vid=vid))) # TODO: use self.season instead?
            ...
            for season in self.seasons:
                for var in self.vars:
                    ...
                    RV = reduced_variable( variableid=var, 
                                           ...
                                           region = self.region,
                                           reduction_function=( lambda x, vid=VID: select_region( x, self.region)) )
                    self.reduced_variables[VID] = RV      
                    VIDs += [VID]              
        ...
        for dt, ft in zip(self.datatype, self.filetables):
            ...
            for month in self.months:
                ...
                RV = reduced_variable( variableid=varid,
                                       ...
                                       region=self.region,
                                       reduction_function=(lambda x, vid=VID, month=VID[2]: reduce2level_region(x, seasons=month, region=self.region, vid=vid) ) )
                VID = id2str(VID)
                self.reduced_variables[VID] = RV      
                VIDs[dt] += [VID]              
...
class amwg_plot_set13(amwg_plot_spec):
    ...
    def var_from_data( self, filetable, varnom, seasonid ):
        """defines the reduced variable for varnom when available in the specified filetable"""
        rv = reduced_variable(
            variableid=varnom, filetable=filetable, season=self.season, region=self.region,
            reduction_function =\
                (lambda x,vid:
                     reduce_time_space_seasonal_regional( x,season=self.season,region=self.region,vid=vid ))
            )
        self.reduced_variables[ rv.id() ] = rv
        return rv.id()
    def var_from_std( self, filetable, varnom, seasonid ):
    ...
    def plan_computation( self, filetable1, filetable2, varnom, seasonid, region ):
        if varnom in filetable1.list_variables_incl_axes():
            vid1 = self.var_from_data( filetable1, varnom, seasonid, self.region )
        elif varnom in self.standard_variables.keys():
            vid1 = self.var_from_std( filetable1, varnom, seasonid, self.region )
        else:
        ...
        if filetable2 is None:
            ...
            vid2 = self.var_from_data( filetable2, varnom, seasonid, self.region )
        elif varnom in self.standard_variables.keys():
            vid2 = self.var_from_std( filetable2, varnom, seasonid, self.region )
        else:
            vid2 = None
    ...
    def plan_computation( self, filetable1, filetable2, varid, seasonid ):
        ...
        for dt, ft in zip(self.datatype, self.filetables):
            for var in self.vars:
                ...
                RV = reduced_variable( variableid=var, 
                                       filetable=ft, 
                                       season=cdutil.times.Seasons(seasonid), region=self.region,
                                       reduction_function=( lambda x, vid=VID_data: select_region( x, region) ) )
                ...
                RV = reduced_variable( variableid=var, 
                                       filetable=ft, 
                                       season=cdutil.times.Seasons(seasonid), region=self.region,
                                       reduction_function=( lambda x, vid=VID_var:MV2.array([x.var()]) ) )  # TODO: add region here
